...
```{r}
packages = c('tidyverse', 'trelliscopejs', 'ggplot2', 'gapminder', 'lubridate', 'dplyr', 'plotly')
for(p in packages){
...
2.  Income is constant for almost all participants. So is education expense and shelter expense. Hence, most volatity in a participant's available balance will be due to changes in other expenses.
...
Next up, I will like to get create a concrete visualisation on the range of values for total, average, maximum and minimum income, expense and balance. We do that using the code chunks below.
